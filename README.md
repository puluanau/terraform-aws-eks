...
| <a name="input_kms"></a> [kms](#input\_kms) | enabled = "Toggle,if set use either the specified KMS key\_id or a Domino-generated one"<br>    key\_id  = optional(list(string), []) | <pre>object({<br>    enabled = optional(bool, true)<br>    key_id  = optional(list(string), [])<br>  })</pre> | `{}` | no |
